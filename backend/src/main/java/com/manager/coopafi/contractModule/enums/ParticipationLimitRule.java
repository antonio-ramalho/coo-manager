package com.manager.coopafi.contractModule.enums;
    ...
    PER_CPF("Por Cpf") {
        ...
        public void validate(Contract contract, Farmer farmer, Price quota) {
            if (quota == null || quota.getValue().compareTo(BigDecimal.ZERO) <= 0) {
                throw new DomainException("A cota do " + farmer.getPerson().getLegalName() + " não pode estar zerada.");
            }

            if (quota.getValue().compareTo(contract.getGlobalLimit().getValue()) > 0) {
                throw new DomainException("A cota do(a) " + farmer.getPerson().getLegalName() + " ultrapassa o limite.");
            }
        }
    },
    PER_CAF("Por Caf") {
